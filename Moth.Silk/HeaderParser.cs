...
public unsafe class HeaderParser : IDisposable
{
    ...
    private Dictionary<string, NamespaceNode> _importsDict { get; } = new Dictionary<string, NamespaceNode>();
    ...
    private List<NamespaceNode> _imports { get => _importsDict.Values.ToList(); }
    private List<TypeNode> _types { get => _typesDict.Values.ToList(); }
    private List<EnumNode> _enums { get => _enumsDict.Values.ToList(); }
        ...
        switch (cKind)
        {
            ...
            case CXCursorKind.CXCursor_StructDecl:
                ...
                if (scopeStatements.Count == 0)
                    break;
                _typesDict.TryAdd(structName, new TypeNode(structName, PrivacyType.Pub, new ScopeNode(scopeStatements), isUnion, new List<AttributeNode>()));
                break;
            case CXCursorKind.CXCursor_EnumDecl:
                ...
                _enumsDict.TryAdd(enumName, new EnumNode(enumName, PrivacyType.Pub, enumFlags, null, new List<AttributeNode>()));
                break;
        ...
        while (result == null)
        {
            ...
            switch (t.kind)
            {
                ...
                case CXTypeKind.CXType_FunctionProto:
                    ...
                    result = new FuncTypeRefNode(TranslateTypeRef(t.ResultType), paramTypes, pointerDepth, false);
                    break;
                case CXTypeKind.CXType_FunctionNoProto:
                    result = new FuncTypeRefNode(TranslateTypeRef(t.ResultType), new List<TypeRefNode>(), pointerDepth, false);
                    break;
                ...
                case CXTypeKind.CXType_Complex:
                    result = new TemplateTypeRefNode("Complex", new List<ExpressionNode>()
                    {
                        TranslateTypeRef(t.ElementType)
                    }, pointerDepth, false);
                ...
                default:
                        ...
                        || t.kind == CXTypeKind.CXType_Enum)
                    {
                        ...
                        {
                            Console.WriteLine($"Creating named definition for unnamed type \"{t}\"...");
                            ...
                            if (isStruct || isUnion)
                            {
                                ...
                                var scopeStatements = new List<StatementNode>();
                                scopeStatements.AddRange(structFields);
                                _typesDict.TryAdd(name, new TypeNode(name, PrivacyType.Pub, new ScopeNode(scopeStatements), isUnion, new List<AttributeNode>()));
                            }
                            else if (isEnum)
                            {
                                _enumFlags = new List<EnumFlagNode>();
                                ...
                                var enumFlags = _enumFlags;
                                _enumsDict.TryAdd(name, new EnumNode(name, PrivacyType.Pub, enumFlags, null, new List<AttributeNode>()));
                            }
                            else
                            {
                                throw new Exception($"Unnamed declaration \"{t.Declaration.kind}\" is unsupported.");
                            }
                        }
                    }
                    else
                    {
                        name = t.kind switch
                        {
                            ...
                            CXTypeKind.CXType_Half
                                or CXTypeKind.CXType_Float16 => "f16",
                            ...
                            _ => throw new NotImplementedException($"Type kind \"{t.kind}\" (\"{t}\") is unsupported.")
                        };
                    }
    ...
    private string ExtractAnonDeclName(string s)
    {
        ...
        var headerName = Path.GetFileNameWithoutExtension(sections[0]).Replace(".", "_").Replace("-", "_");
